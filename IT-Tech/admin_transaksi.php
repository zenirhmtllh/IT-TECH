  ...
  <div class="collapse navbar-collapse" id="navbarNav" style="padding-left:5%;">
    <ul class="navbar-nav">
      <li class="nav-item active">
        <a class="nav-link" href="#" style="font-size: 25px;">Transaksi<span class="sr-only">(current)</span></a>
      </li>
    </ul>
			...
			<div class="col-10">
				...
				<table  class="table">
					...
					<tbody>
						<?php $nomor=1;?>
						<?php
						$datp = $koneksi->query("SELECT * FROM pembelian");
						while($datapembelian = $datp-> fetch_assoc()){
						?>
						<tr>
							<td><?php echo $nomor;?></td>
							<td><?php echo $datapembelian["id_pembelian"];?></td>
							<td><?php echo $datapembelian["id_pelanggan"];?></td>
							<td><?php echo $datapembelian["nama_kota"];?></td>
							<td><?php echo $datapembelian["tarif"];?></td>
                            <td><?php echo $datapembelian["alamat_pengiriman"];?></td>
                            <td><?php echo $datapembelian["tanggal_pembelian"];?></td>
                            <td>Rp.<?php echo number_format($datapembelian["total_pembelian"]);?></td>
                            <td><?php echo $datapembelian["status"];?></td>
                            </tr>
						<?php $nomor++;?>
                        <?php } ?>
						</tbody>
